class TripleExponentialMovingAverage {
    ...
    calculate(candles) {
        ...
        const ema3 = this._calcEmaFromValues(ema2, this.period);
        for (let i = 0; i < candles.length; i++) {
            if (i < ema1.length && i < ema2.length && i < ema3.length && ema1[i] !== null && ema2[i] !== null && ema3[i] !== null)
                this.values.push(3 * ema1[i] - 3 * ema2[i] + ema3[i]);
    ...
    static calculate(candles, period) {
        const tema = new TripleExponentialMovingAverage(period);
