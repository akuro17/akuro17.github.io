...
function initMermaidDrag() {
    ...
    container.addEventListener('mousedown', (e) => {
        container.focus(); // Explicitly set focus since preventDefault stops it
        e.preventDefault();
        isDragging = true;
    ...
    container.addEventListener('keydown', (e) => {
        ...
        switch (e.key) {
            ...
            case 'ArrowDown':
                ...
                break;
            case '+':
            case '=': // Support + without Shift often sharing = key
                zoomAction('in');
                break;
            case '-':
            case '_':
            ...
            case 'R':
                resetView();
                // Prevent check for updateCanvasTransform twice since resetView calls it
                e.preventDefault();
                return;
            default:
                return; // Early exit for unhandled keys
        }
...
// ==========================================
// [IaI] Logic moved to indicator.js for lazy loading support
/*
...
function updateButtonStates() {
    ...
    const langCsharp = document.getElementById('btn-lang-csharp');
    const langPython = document.getElementById('btn-lang-python');
    if (langCsharp) langCsharp.classList.remove('btn-lang-active');
    if (langPython) langPython.classList.remove('btn-lang-active-python');

    // Highlight active language
    if (currentLang === 'csharp' && langCsharp) {
        langCsharp.classList.add('btn-lang-active');
    } else if (currentLang === 'python' && langPython) {
        langPython.classList.add('btn-lang-active-python');
    }

    // Reset type buttons
    const typeSimple = document.getElementById('btn-type-simple');
    const typeDetailed = document.getElementById('btn-type-detailed');
    if (typeSimple) typeSimple.classList.remove('btn-type-active');
    if (typeDetailed) typeDetailed.classList.remove('btn-type-active');

    // Highlight active type
    const activeTypeBtn = document.getElementById(`btn-type-${currentType}`);
    if (activeTypeBtn) activeTypeBtn.classList.add('btn-type-active');
}

function toggleCodePanel() {
    const overlay = document.getElementById('code-panel-overlay');
    if (overlay) overlay.classList.toggle('panel-hidden');
}
...
// Only define PlaygroundController if not already defined by inline scripts
// Inline scripts in generated pages may define a more complete version with generateSample
...
2023-01-15,159.00,160.00,155.00,156.00,1400000`;
        ...
        parseData: function (text) {
            ...
            lines.forEach((line, i) => {
                ...
                // 3. First column is a date string (contains - or / separators)
                const firstPart = parts[0];
                const looksLikeDateString = /\d{4}[-/]\d{2}[-/]\d{2}/.test(firstPart) ||
                    /\d{2}[-/]\d{2}[-/]\d{4}/.test(firstPart) ||
                    (firstPart.includes('-') && firstPart.length >= 8) ||
                    (firstPart.includes('/') && firstPart.length >= 8);

                // If it looks like OHLCV (Date string at start, right column count)
                if (parts.length >= 5 && parts.length <= 8 && looksLikeDateString) {
                    data.push({
                        date: parts[0],
...
function initApp() {
    ...
    // updateCodeVisibility(); // Initial Code Panel State (Handled by indicator.js)
